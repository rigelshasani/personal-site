...
export function SideNavigation({ isCollapsed }: SideNavigationProps) {
  ...
  return (
    <aside className="fixed left-0 top-16 h-[calc(100vh-4rem)] w-36 bg-bg transition-all duration-300 z-40">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`block text-center text-xl font-bold transition-colors ${
                  isActive
                    ? 'text-accent'
